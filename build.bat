...
::    D  huge overhaul / milestone release        (option 1 — most significant)
::    C  fairly important / significant update    (option 2)
::    B  tested and confirmed stable              (option 3)
::    A  every new build, likely buggy            (option 4 — most frequent)
::
...
echo   1. Overhaul  ^(%CUR_VER% -^> %VER_D%^)  -- huge milestone release
echo   2. Important ^(%CUR_VER% -^> %VER_C%^)  -- fairly significant update
echo   3. Stable    ^(%CUR_VER% -^> %VER_B%^)  -- tested and confirmed stable
echo   4. Build     ^(%CUR_VER% -^> %VER_A%^)  -- new build, may be buggy
...
if "%UPDATE_TYPE%"=="1" goto do_d
if "%UPDATE_TYPE%"=="2" goto do_c
if "%UPDATE_TYPE%"=="3" goto do_b
if "%UPDATE_TYPE%"=="4" goto do_a
...
:do_a
set SKIP_RELEASE=1
python -c "p=open('version.txt').read().strip().split('.');p[0]=str(int(p[0])+1);v='.'.join(p);open('version.txt','w',encoding='utf-8').write(v);print('Build: %CUR_VER% ->',v)"
call :patch_py
goto build

:do_b
python -c "p=open('version.txt').read().strip().split('.');p[1]=str(int(p[1])+1);p[0]='0';v='.'.join(p);open('version.txt','w',encoding='utf-8').write(v);print('Stable: %CUR_VER% ->',v)"
call :patch_py
goto build

:do_c
python -c "p=open('version.txt').read().strip().split('.');p[2]=str(int(p[2])+1);p[1]='0';p[0]='0';v='.'.join(p);open('version.txt','w',encoding='utf-8').write(v);print('Important: %CUR_VER% ->',v)"
call :patch_py
goto build

:do_d
python -c "p=open('version.txt').read().strip().split('.');p[3]=str(int(p[3])+1);p[2]='0';p[1]='0';p[0]='0';v='.'.join(p);open('version.txt','w',encoding='utf-8').write(v);print('Overhaul: %CUR_VER% ->',v)"
call :patch_py
goto build
...
:: Stage all source files including build.bat itself
git add version.txt main.py user_config.py voice_controls.py manage_apps.py settings_window.py .gitignore build.bat
