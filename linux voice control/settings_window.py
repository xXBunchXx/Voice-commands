...
Linux version: uses Sans/Monospace fonts, .desktop shortcut creation.
"""
import threading
...
def _combo_str(held: set) -> str:
    order = ["super", "ctrl", "shift", "alt", "altgr"]
...
def _lbl(parent, text, fg=FG, font=("Sans", 9), **kw):
...
def _context_display_maps():
    proc_names = user_config.get_proc_names()
...
class SettingsWidget(tk.Frame):
    ...
    def _tab_engine(self, nb):
        ...
        tk.Checkbutton(card3, text="Show overlay when a command fires",
                       ...
                       font=("Sans", 9)).grid(row=0, column=0, columnspan=2, sticky="w")
        ...
        tk.Checkbutton(shortcut_row,
                       ...
                       bg=CARD, fg=FG, selectcolor=ENTRY_BG,
                       activebackground=CARD, activeforeground=FG,
                       font=("Sans", 9)).pack(side="left", anchor="w")
        shortcut_btn.config(state="disabled")

        self._make_save_btn(frame, self._save_engine)

    def _create_shortcut(self):
        """Create a .desktop file in ~/Desktop for Echo."""
        import sys
        exe_path = pathlib.Path(sys.executable)
        desktop  = pathlib.Path.home() / "Desktop"
        desktop.mkdir(parents=True, exist_ok=True)
        lnk_path = desktop / "Echo.desktop"
        icon_png  = exe_path.parent / "icon.png"
        icon_str  = str(icon_png) if icon_png.exists() else "utilities-terminal"
        try:
            lnk_path.write_text(
                "[Desktop Entry]\n"
                "Version=1.0\n"
            ...
            lnk_path.chmod(0o755)
            self._flash(f"✓  Shortcut created: {lnk_path}")
    ...
    def _build_volume_section(self, parent):
        ...
        _lbl(card, 'Say  "volume up <word>"  or  "volume down <word>"  to change by that amount.',
             fg=MUTED, font=("Sans", 8)).pack(anchor="w", pady=(0, 8))
    ...
    def _tab_commands(self, nb):
        ...
        _lbl(frame,
             ...
             fg=MUTED, font=("Sans", 8), justify="left").pack(
        ...
        for group_name, keys in groups:
            ...
            if is_app_ctrl:
                _lbl(card,
                     ...
                     fg=MUTED, font=("Sans", 8), wraplength=560,
            ...
            w = 18
            _lbl(hdr, "Action", fg=FG, font=("Sans Bold", 8),
                 width=w, anchor="w").pack(side="left")
            _lbl(hdr, "Trigger word(s)", fg=FG, font=("Sans Bold", 8),
                 anchor="w").pack(side="left")
            _lbl(hdr, ms_label, fg=FG, font=("Sans Bold", 8),
    ...
    def _tab_context(self, nb):
        ...
        _lbl(mode_row, "Mode:", fg=FG, font=("Sans Bold", 9)).pack(side="left")
        ...
        _lbl(mode_row, "Say \"set mode <name>\" to switch.",
             fg=MUTED, font=("Sans", 8)).pack(side="right")
        ...
        _lbl(top,
             ...
             "(built-in group, custom group, or any process name).",
             fg=MUTED, font=("Sans", 8)).pack(side="left")
        ...
        self._ctx_canvas.bind("<Button-4>",
            lambda e: self._ctx_canvas.yview_scroll(-1, "units"))
        self._ctx_canvas.bind("<Button-5>",
            lambda e: self._ctx_canvas.yview_scroll(1, "units"))
    ...
    def _reload_mode_groups(self):
        ...
        tk.Label(self._mode_groups_frame,
                 ...
                 bg=CARD, fg=FG, font=("Sans Bold", 9)).pack(anchor="w")
    ...
    def _import_cmds(self):
        ...
        if not messagebox.askyesno(
                ...
                "Existing commands with the same phrase + context will be overwritten.",
                parent=self.winfo_toplevel()):
            return
        cmds = self._mode_commands()
    ...
    def _show_cmd_editor(self, *, phrase="", context="browser",
        ...
        imp_lbl = tk.Label(imp_bar, text="", bg=CARD, fg=AMBER,
                           font=("Sans", 8), justify="left", wraplength=380)
        ...
        field_row("Voice phrase  (what you say)",
                  lambda f: tk.Entry(f, textvariable=phrase_var,
                                     ...
                                     relief="flat", font=("Sans", 10), bd=4))

        def _ctx_widget(f):
            cb = ttk.Combobox(f, textvariable=context_var, state="normal",
                              values=ctx_display_values, font=("Sans", 10))
            return cb
        field_row("Context", _ctx_widget)
        tk.Label(body,
                 text="  Built-in: browser · explorer · editor · any\n"
                      "  Your groups appear here too — or type any process name (e.g. blender)",
                 bg=CARD, fg=MUTED, font=("Sans", 8), justify="left").pack(anchor="w")
        ...
        tk.Label(speed_row, text="0 = use global Response speed; lower (e.g. 40) = near-instant",
                 bg=CARD, fg=MUTED, font=("Sans", 8)).pack(side="left", padx=(8, 0))
        ...
        tk.Label(mode_row, text="Action:", bg=CARD, fg=FG,
                 font=("Sans Bold", 9)).pack(side="left", padx=(0, 12))
        ...
        tk.Label(sc_frame,
                 text="Shortcut  (e.g.  ctrl+w  ·  f5  ·  super+l  ·  ctrl+shift+t)",
                 bg=CARD, fg=MUTED, font=("Sans", 8)).pack(anchor="w", pady=(0, 4))

        sc_inp_row = tk.Frame(sc_frame, bg=CARD)
        sc_inp_row.pack(fill="x")
        sc_entry = tk.Entry(sc_inp_row, textvariable=shortcut_var,
                            bg=ENTRY_BG, fg=FG, insertbackground=FG,
                            relief="flat", font=("Monospace", 10), bd=4)
        ...
        tk.Spinbox(rep_row, from_=1, to=999, textvariable=repeat_var, width=5,
                   ...
                   relief="flat", font=("Sans", 10)).pack(side="left", padx=(6, 0))
        tk.Label(rep_row, text="times", bg=CARD, fg=MUTED,
                 font=("Sans", 9)).pack(side="left", padx=(6, 0))

        tk.Label(mac_frame, text="Steps:", bg=CARD, fg=FG,
                 font=("Sans Bold", 9)).pack(anchor="w")
        ...
        steps_cv.bind("<Button-4>", lambda e: steps_cv.yview_scroll(-1, "units"))
        steps_cv.bind("<Button-5>", lambda e: steps_cv.yview_scroll(1, "units"))
        ...
        def _redraw_steps():
            ...
            if not steps:
                tk.Label(steps_inner,
                         ...
                         bg=ENTRY_BG, fg=MUTED, font=("Sans", 8),
        ...
        def _make_step_row(idx, step):
            ...
            f.bind("<Button-4>", lambda e: steps_cv.yview_scroll(-1, "units"))
            f.bind("<Button-5>", lambda e: steps_cv.yview_scroll(1, "units"))

            tk.Label(f, text=f"{idx+1:2d}.", bg=row_bg, fg=MUTED,
                     font=("Monospace", 9), width=3).pack(side="left")
            ...
            tk.Button(f, text=step["type"].upper(), command=_toggle_type,
                      ...
                      activeforeground="#fff", relief="flat",
                      font=("Sans Bold", 8), padx=6, pady=2,
            ...
            if step["type"] == "press":
                ...
                e = tk.Entry(f, textvariable=key_var, bg=e_bg, fg=FG,
                             insertbackground=FG, relief="flat",
                             font=("Monospace", 9), bd=2, width=18)
                ...
                e = tk.Entry(f, textvariable=ms_var, bg=e_bg, fg=FG,
                             insertbackground=FG, relief="flat",
                             font=("Monospace", 9), bd=2, width=7)
                e.pack(side="left", padx=(0, 4))
                e.bind("<MouseWheel>", _scroll_body)
                tk.Label(f, text="ms", bg=row_bg, fg=MUTED,
                         font=("Sans", 8)).pack(side="left", padx=(0, 8))
        ...
        rec_b = tk.Button(add_row, text="🔴  Record", command=_toggle_record,
                          bg=MUTED, fg="#fff", activebackground=MUTED,
                          activeforeground="#fff", relief="flat",
                          font=("Sans Bold", 9), padx=8, pady=5, cursor="hand2")
        rec_b.pack(side="left")
        rec_btn_r[0] = rec_b

        tk.Label(mac_frame,
                 text="Record: click here to focus, then press key combos — each combo = one step.",
                 bg=CARD, fg=MUTED, font=("Sans", 8)).pack(anchor="w", pady=(4, 0))
    ...
    def _tab_groups(self, nb):
        ...
        _lbl(top,
             ...
             'e.g. a "music" group with Spotify + Rhythmbox → one set of commands for both.',
             fg=MUTED, font=("Sans", 8), justify="left").pack(side="left")
        ...
        for w in (self._grp_canvas, self._grp_inner):
            w.bind("<MouseWheel>",
                   lambda e: self._grp_canvas.yview_scroll(-1*(e.delta//120), "units"))
            w.bind("<Button-4>",
                   lambda e: self._grp_canvas.yview_scroll(-1, "units"))
            w.bind("<Button-5>",
                   lambda e: self._grp_canvas.yview_scroll(1, "units"))
    ...
    def _reload_groups_list(self):
        ...
        if not groups:
            tk.Label(self._grp_inner,
                     ...
                     bg=BG, fg=MUTED, font=("Sans", 9), pady=20).pack(anchor="w")
            return
        ...
        for gname, procs in sorted(groups.items()):
            ...
            tk.Label(top_row, text=f"👥  {gname}", bg=CARD, fg=FG,
                     font=("Sans Bold", 11)).pack(side="left")
    ...
    def _show_group_editor(self, *, name="", procs=None):
        ...
        tk.Label(card, text="  Commands targeting this group fire when any of these apps is focused.",
                 bg=CARD, fg=MUTED, font=("Sans", 8)).pack(anchor="w", pady=(0, 6))
        ...
        for w in (members_cv, members_frame):
            w.bind("<MouseWheel>", _mscroll)
            w.bind("<Button-4>", lambda e: members_cv.yview_scroll(-1, "units"))
            w.bind("<Button-5>", lambda e: members_cv.yview_scroll(1, "units"))

        all_procs = user_config.get_proc_names()
        member_vars: dict[str, tk.BooleanVar] = {}

        if not all_procs:
            tk.Label(members_frame, text="No apps added yet — add apps in the 📦 Apps tab first.",
                     bg=ENTRY_BG, fg=MUTED, font=("Sans", 8)).pack(anchor="w")
        else:
            for app_name in sorted(all_procs.keys()):
                ...
                row.bind("<Button-4>", lambda e: members_cv.yview_scroll(-1, "units"))
                row.bind("<Button-5>", lambda e: members_cv.yview_scroll(1, "units"))
    ...
    def _tab_audio(self, nb):
        ...
        _lbl(top,
             ...
             fg=MUTED, font=("Sans", 8), justify="left", wraplength=520).pack(side="left")
    ...
    def _reload_audio_list(self):
        ...
        try:
            ...
            tk.Label(self._audio_inner, text=f"Couldn't read audio devices: {e}",
                     bg=CARD, fg=RED, font=("Sans", 9)).pack(anchor="w", padx=8, pady=8)
            return
        if not devices:
            tk.Label(self._audio_inner, text="No output devices found.",
                     bg=CARD, fg=MUTED, font=("Sans", 9)).pack(anchor="w", padx=8, pady=8)
            return
    ...
    def _tab_models(self, nb):
        ...
        _lbl(top,
             ...
             fg=MUTED, font=("Sans", 8), justify="left").pack(anchor="w")
    ...
    def _build_model_card(self, parent, m: dict, scroll_fn):
        ...
        tk.Label(hdr, text=name, bg=CARD, fg=FG,
                 font=("Sans Bold", 10)).pack(side="left")
        ...
        if is_default:
            ...
            tk.Checkbutton(
                noise_row,
                text="Enable noise filter  (uses the small model to remove random words heard at the start of commands)",
            ...
            hint_lbl = tk.Label(noise_row, text="", bg=CARD, font=("Sans", 8))
            hint_lbl.pack(side="left", padx=(8, 0))
            _update_noise_hint()
            card.bind("<Visibility>", lambda e: _update_noise_hint())
            self._noise_hint_refresh = _update_noise_hint

        status_lbl = tk.Label(card, text="", bg=CARD, fg=MUTED, font=("Sans", 8))
        status_lbl.pack(anchor="w")

        btn_row     = tk.Frame(card, bg=CARD)
        btn_row.pack(fill="x", pady=(4, 0))
        progress_lbl = tk.Label(btn_row, text="", bg=CARD, fg=FG, font=("Sans", 8))
        ...
        use_btn = tk.Button(btn_row, text="✓ Use This Model",
                            ...
                            font=("Sans Bold", 9), padx=8, pady=4,
                            cursor="hand2",
    ...
    def _download_model(self, m: dict, progress_lbl, status_lbl, on_done=None):
        ...
        def _run():
            try:
                ...
                self.after(0, lambda: status_lbl.config(text="Extracting…", fg=AMBER))
                ...
                self.after(0, lambda: status_lbl.config(text="✓ Downloaded", fg=GRN))
    ...
    def _load(self):
        try:
            ...
            self._set_spin(self._response_spin,
                           int(round(user_config.get_response_delay() * 1000)))
